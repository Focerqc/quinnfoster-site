    ...
    for (let c = 0; c < cols; c++) {
      ...
      for (let r = 0; r < rows; r++) {
        ...
        switch (presetName) {
          ...
          case 'custom4':
            // 🎯 Precision Target Ring Window Slots (with continuous vertical spine backbone)
            const c4_spine = Math.abs(Math.cos(twistedAngle * 3));
            if (c4_spine > 0.85) {
              v = 0.0; // Solid structural backbone rail connecting top to bottom
            } else {
          ...
          case 'bricks':
            // 🧱 Brick Lattice Window Cutouts (with continuous structural pillars)
            ...
            v = ((brickC % 3 === 0) && (brickR % 2 === 0)) ? 1.0 * contrast : ((brickR % 4 === 0) ? 0.35 * contrast : 0.0);
            break;
        }
